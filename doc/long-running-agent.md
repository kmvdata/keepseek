...
命令菜单末尾的 Goal checkbox 是范围明确、具有可验证停止条件的持久任务入口。它开启当前 session 持久化的 Goal composer mode；用户直接在输入框提交目标、引用和 Skills，提交不会走普通 `sendPrompt`。宿主通过已配置的 `proposal` 模型发起一次无工具严格 JSON 请求，同时判定目标是否可直接采用、解释理由、必要时给出不扩展范围的规范化目标，并生成与最终目标绑定的工作项、验收证据、工作区相对 include/exclude scope 和当前可用 `compile | lint | test`。输出严格验证，失败则回退宿主保守 proposal。创建前请求只记录普通 session 的 `subagent` 用量，不创建 Goal、ChatMessage、权限、副作用或 Goal 预算账本。

消息列表与输入框之间的常驻区域先显示确认卡：理由和原目标/规范化目标差异默认展开，可选择工作项、采用建议、按原目标使用保守 proposal、重新生成、进入创建高级设置或内联确认放弃。高级设置 Dialog 只保留可编辑目标、工作项/验收、validation、scope、四类预算、恢复策略和消息预览；编辑目标会立即作废旧 assessment/proposal。采纳后，最终可见 objective、引用安全处理后的 canonical objective、V2 Contract objective 来自同一次选择，`visibleContent`、`expandedContent` 与附带确定性 contract tail 的 `providerContent` 各生成并持久化一次；Goal Store、lease 和首个 v3 RunCheckpoint 成功写入后，才允许首个正式 Provider 请求。

Goal 创建后，确认卡互斥替换为常驻进度卡，而不是追加到 transcript 或打开运行管理 Dialog。卡片原位刷新状态、步骤、工作项、验收/validation、有效执行时间、请求/review/分币种费用、模型与审批模式，并提供 Pause/Resume/Stop/Clear、append-only Amend、人工 criterion、trace 与 Debug。当前 attempt 同时投影为 view-only assistant 消息，复用普通对话的增量文本/推理渲染，但对象只保存在 Extension Host 内存并在状态投影时临时追加，既不追加 `ChatSession.messages`，也不进入三协议 replay。候选审查期间保留已收束的本轮输出，下一 attempt 替换它，最终 completed ChatMessage 落盘前将其移除。显式 Resume 先在卡片回传 pending，再复核当前 session/source/model/checkpoint/Trust/授权/不确定副作用和 lease；它不重复 activation recovery，也不会再次清空本次宿主内已经有效的 validation/criterion evidence。Stop 只进入 `stopped` 并保留所有证据和结果；Clear 只在终态显示，成功后在一个一致状态快照中发送显式 `goal: null`、移除卡片并关闭绑定 session 的 Goal composer mode，失败则保留终态卡和模式。修订不重写原目标或历史，并使旧验证/完成审查失效。

Goal 状态由 `src/agent/goals/goalCoordinator.ts` 单独拥有。一次 `AgentRunner` attempt 没有工具调用时返回 `candidate_final`，不会直接完成 checkpoint 或把候选 assistant 写进聊天历史。宿主先检查 lease、冻结身份、Workspace Trust、授权、TaskPlan、criteria、最后 mutation 之后的验证、ChangeSet/DraftRun/approval/tool/subagent 终态和预算；再调用隔离、无工具、无写入/执行能力的 completion reviewer。reviewer 决定绑定 contract/revision/candidate/evidence manifest/mutation revision，提交 completed 前全部重验。只有纯机器可验证 criteria 才允许 reviewer 不可用 fallback；正费用上限无法对 reviewer 计价时不允许 fallback。manual criterion 必须由用户在常驻 Goal 进度卡中明确确认。
