...
### 2.2.1 Goal composer 创建与宿主边界

Goal 入口位于 `/` 命令菜单末尾，是持久化到各 session 的 composer 模式；`goalComposerMode` 只参与宿主/UI 状态，不进入任何 Provider projection。开启后，用户直接在 composer 提交目标和引用/Skill 元数据，Webview 不发送普通 `sendPrompt`。Extension Host 用 `proposal` 配置发起一次可取消、无工具的严格 JSON 请求，在同一响应中给出 `ready | needs_normalization`、理由、原目标、规范化目标和与规范化目标绑定的 `GoalProposalV1`。输出需通过精确字段、长度、安全文本、workspace-relative scope、可用 validation、依赖无环和 proposal hash 校验；临时 assessment/proposal 只保存在宿主易失内存，不进入 ChatMessage、GoalStore 或 replay。失败时保留宿主验证的保守 proposal；该请求用量只归入普通 session 的 `subagent` 分类，Goal 账本尚未建立。

消息列表与 composer 之间有独立常驻 Goal 区域：创建前先显示可取消的准备卡，再原位显示判定理由、默认展开的原目标/规范化目标差异、工作项选择和模型；无 proposal、输入校验失败、模型失败或取消也必须保留可恢复的宿主状态卡，不能只显示状态栏。提案流只投影阶段与累计字符数，不展示原始 JSON。采纳建议、按原目标使用保守 proposal、重新生成或内联确认放弃都在卡片完成。创建 Dialog 只负责目标编辑、工作项、validation、scope、预算、恢复策略和可见消息预览；目标编辑立即使旧 assessment/proposal 失效。创建后同一区域互斥替换为进度卡，Pause/Resume/Stop/Clear、append-only Amend、人工 criterion、trace 和 Debug 都在卡片操作，Dialog 不再承担运行态管理。统一的宿主 `goalUi` 投影驱动卡片、状态栏、菜单 checked 状态和 composer 只读/placeholder；Goal 位于另一 session 时，菜单项只导航回其不可变绑定 session。开始操作把选定 proposal、workspace-relative scope、required validations、预算、恢复策略和冻结 source/model/provider/endpoint 组装为 `GoalContractV2`；canonical serializer 固定字段/数组顺序与 LF，以完整序列化字节计算 hash。Goal ID、时间、绝对路径、凭据、lease owner 与 runtime 值不进入 Provider contract tail。
...
`GoalCoordinator` 是唯一续跑状态机；Provider 只做入口、事件转发和裁剪 view model。`GoalStore` 使用 index、不可变 snapshot、单调 `storageRevision` 乐观并发校验与 journal shard；`GoalLease` 为 file global storage 提供 heartbeat/expiry/fencing。启动或恢复等待 lease 时，Stop/interrupt/amend/dispose 会递增生命周期 generation，迟到的 acquire 必须释放且不得覆盖新状态或 dispatch。`GoalReplay` 维护三协议内部 continuation；`GoalCompletionReviewService` 执行硬检查和隔离 reviewer。Goal 状态与 checkpoint、lease、隐藏 reviewer 输入或大 evidence 不发送给 Webview。当前 Goal attempt 的 Provider delta 通过独立的内存 assistant 投影复用普通 transcript 流式渲染；归一化活动状态另进入有界、view-only 的对话列表活动面板。两者都不属于 session 历史，完成候选落盘、后续 replay 与缓存投影都不读取这些对象。显式 Resume 有独立 pending/success/error 回执并只执行当前安全复核；若当前显示的是另一会话，Resume 先按不可变 `sessionId + workspaceKey` 重载 Goal 原会话及其 ChangeSet/DraftRun/运行上下文，绝不把 Goal 历史移植到当前会话，所属会话缺失或 workspace 绑定不符时 fail-closed。activation recovery 仅在重新激活时执行一次。Stop 保留证据、Draft 与结果并进入 `stopped`；Clear 只对终态开放，宿主抑制 coordinator 的中间状态推送，在 Store 清理成功并关闭绑定 session 的 composer mode 后只发送一个一致的最终快照，失败则保持终态 Goal 和模式。
