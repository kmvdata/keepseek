...
export class InteractionTraceLogService {
  ...
  public createRunTrace(): AgentInteractionTrace {
    ...
    if (!settings.enabled || this.globalStorageUri.scheme !== 'file') {
    ...
    const filePath = path.join(this.globalStorageUri.fsPath, TRACE_ROOT_DIR, day, fileName);
    void this.cleanupExpiredLogs(settings);

    return new JsonlInteractionTrace(runId, filePath, settings);
  }
