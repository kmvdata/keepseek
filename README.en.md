...
Type the objective in the input first, then click the `G` button beside `/`. KeepSeek uses the configured `proposal` subagent model without tools to generate acceptance criteria, evidence requirements, workspace-relative scope, and available validations. Generation can be cancelled; a failure leaves conservative editable defaults. The common confirmation stays compact, while budgets, recovery, and fallback evidence live under Advanced settings. No Goal or chat message is created until you choose Start Goal; the prefill request counts only toward session usage. Once started, an independently rendered card in the conversation transcript updates the current step, criteria and validation progress, active execution, and model-request usage. A temporary assistant entry also streams the current Provider reasoning and text like an ordinary question, but it is UI-only and never enters chat history or internal replay. **Resume Goal** immediately shows its recovery check; failures remain visible in the dialog and the action becomes available again. Use **View / Manage** on the card for other actions. Hover over `G` for the compact status, and preparation can be cancelled immediately. If the model returns final text early, KeepSeek checks side-effect terminal states, validations after the latest mutation, criterion evidence, and an isolated completion reviewer before completing or continuing the same logical task.

Goals do not relax safety. Ask mode still waits for your real Apply/Run action, model review remains bound to the exact action hash, and auto approve still records an explicit host-policy approval without model review. After a reload or crash, running state becomes interrupted; old permits, temporary external authorization, and in-memory approval queues are not revived. If Resume encounters the previous Host's short unexpired lease, the dialog shows the wait and safely takes it over after the remaining TTL (at most about 30 seconds); Stop cancels that wait. A different VS Code window that is still renewing the lease remains authoritative and its Goal record is not rewritten. An uncertain file or command outcome requires human verification and is never automatically retried. The legacy “repair until compile/lint/test passes” entry now opens a prefilled Goal.

If a desktop Node Extension Host exposes global storage through a non-`file:` URI, KeepSeek uses the absolute native storage path supplied alongside it to provide the same atomic lease semantics. A different URI scheme alone no longer blocks recovery. Environments without a reliable native path still remain paused to prevent double execution.
