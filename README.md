...
### 6. 为什么 KeepSeek 不提供 Goal 模式

所谓 Goal 模式，通常是指：用户给出目标与验收条件，Agent 持续执行，直到交付经宿主验证的结果，或指出需要用户采取什么行动才能继续。它真正新增的产品承诺不是“多跑几轮”，而是**用户不必持续盯守，系统也能给出可信且可处置的终态**。

这个承诺只有同时满足以下条件才成立：任务确实需要跨轮自主推进；完成标准能够被宿主可靠验证；执行可以在明确的权限与资源上限内安全恢复；而且这些能力不能被常规模式或更具体的工作流等价替代。缺少任何一项，通用 Goal 模式要么不可靠，要么没有必要。

一般程序是否满足任意自然语言目标，不存在通用、完备的自动判定方法；测试通过、编译成功等检查只能验证事先约定的局部标准。轮次耗尽或无进展熔断虽然能安全停止资源消耗，却没有完成任务，也未必能给出用户可以直接解除的阻塞，因此不能兑现上述终态承诺。

可以把这个边界拆成下图的两个维度：横轴表示任务是否复杂到需要跨轮自主推进，纵轴表示验收结果能否由宿主可靠验证。四个象限分别对应四种不同的处理方式：

```mermaid
quadrantChart
    title 通用 Goal 模式的适用性
    x-axis 任务简单 --> 任务复杂
    y-axis 验收不可由宿主验证 --> 验收可由宿主验证
    quadrant-1 专用、受控、可审计工作流
    quadrant-2 常规模式 + 审批
    quadrant-3 保留用户判断与交互
    quadrant-4 无法可靠验收
```

图中左侧的简单任务不需要额外的跨轮调度：验收明确时使用常规模式和当前审批方式即可，验收主观时则应保留用户判断。右下的复杂但不可验证任务无法可靠区分“完成”与“模型认为完成”，不适合承诺无人盯守。只有右上的复杂且可验证任务适合持续自动推进，但它更适合由测试、编译、检查和受控后台修复等专用工作流承载，而不是抽象成覆盖任意目标的通用模式。

因此，四个象限中没有一个必须依赖通用 Goal 模式。KeepSeek 不提供这一模式，也不承诺“无人盯守直到任意目标完成”。长任务继续由上下文续跑、子代理、模型审批或自动批准等现有机制推进；能够机械验收的闭环则以明确、受限、可审计的专用工作流提供。这样可以保留长任务能力，同时避免引入无法普遍兑现的完成承诺、额外费用和新的跨轮状态复杂度。
