...
先在输入框写下目标，再点击“/”旁的 `G` 按钮。KeepSeek 会用已配置的 `proposal` 子代理模型（无工具）自动生成验收条件、证据要求、工作区相对范围和可用验证项；生成可取消，失败时保留可编辑的保守默认项。常用内容直接确认即可，预算、恢复策略与证据回退收在“高级设置”中。在你点击“开始 Goal”前，不会创建 Goal 或聊天消息；自动预填模型请求只计入当前会话用量。开始后，聊天对话列表会用独立状态卡实时显示当前步骤、验收/验证进度、有效执行时间和模型请求数，并像普通提问一样用临时 assistant 消息流式显示本轮 Provider 的推理与正文；这些运行中内容仅用于界面反馈，不会写入聊天历史或内部 replay。点击卡片上的“查看 / 管理”可打开操作弹窗；“恢复 Goal”会立即显示恢复检查进度，失败原因会留在弹窗并恢复按钮可用。悬浮 `G` 也可快速查看状态，准备阶段可立即取消。模型过早给出最终文字时，KeepSeek 会先核对文件/命令终态、最后一次修改后的验证、验收证据与隔离 completion reviewer，再决定完成或在同一逻辑任务中继续。

Goal 不改变安全边界：请求批准模式仍等待你真实 Apply/Run；模型审批继续绑定精确 actionHash；自动批准仍记录“未经模型审查”的宿主策略。Reload 或崩溃后，运行态先标记为 interrupted，旧 permit、临时外部授权和内存审批队列不会复活。点击恢复若碰到上一 Host 尚未到期的短期租约，会在弹窗显示等待并于剩余租约时间结束后安全接管（最长约 30 秒）；可随时 Stop 取消。若另一 VS Code 窗口仍在运行同一 Goal，则保持阻止且不改写它的状态。文件或命令终态不确定时必须人工核实，绝不会自动重跑。旧“持续修复直到 compile/lint/test 通过”入口现在只是预填 Goal 的兼容入口。

桌面 Node Extension Host 若为全局存储返回非 `file:` URI，KeepSeek 会使用 VS Code 同时提供的绝对本机存储路径建立同等级的原子租约；不会仅因 URI scheme 不同而拒绝恢复。确实没有可靠本机路径的环境仍保持暂停，避免双重执行。
