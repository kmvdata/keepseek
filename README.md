...
- **请求批准**（默认）：文件写入、删除、Shell 命令和受控验证逐项请求批准。
- **帮我批准**：本会话自动批准文件访问、采纳 DraftEdit、执行 DraftRun，并将真实结果发回模型继续任务；不需要守着审批窗口。可以在任务运行中切回“请求批准”或点击停止，撤销后续自动操作。

“帮我批准”按用户委托直接自动批准，不调用 Codex 的独立审批模型。操作仍保留修改/命令卡片、执行输出和回滚记录；脏编辑器、文件冲突、未信任工作区等检查继续有效。重启只保留模式选择，不自动执行旧队列。已有任务的工具调用轮次耗尽后可自动开启后续轮；明确设置的时间限制、上下文上限和真实错误仍可能结束任务。
...
受控验证工具仅运行固定的 `compile` / `lint` / `test`；其他命令通过 DraftRun 审批执行。失败后读取 Problems、准备修复 DraftEdit，按当前审批模式处理；修复真正 Apply 后再继续验证。
