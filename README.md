...
- 用量先按**账号 + 模型**归属，再区分主请求、摘要、重试、续接与后台任务；
...
KeepSeek 也支持隔离的“主模型 + 子代理”工作模式：主模型可把独立调查、审查或待确认修改提案交给受限子会话，并行完成后只把有界最终结果带回当前对话，从而避免把父历史与子代理中间工具轨迹反复发送给模型。子代理模型可在“账号管理”中全局固定；架构、安全边界和 Profile 格式见 [SUBAGENTS.md](SUBAGENTS.md)。
