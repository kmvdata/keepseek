...
- **启动快、成本低**：冷启动立即可用，常用设置启动即就绪；用量估算带缓存、提示缓存命中率高，长对话更省 token、更省钱；
...
配合账号体系，KeepSeek 还提供一系列省心的模型管理细节：

- **全局默认模型**：可将任一可用模型“设为默认”，未显式选择时自动采用，且跨工作区保持一致；默认模型不可用时自动清除，不会静默卡住任务；
- **模型别名**：为常用账号或模型自定义别名，切换时一眼认出；
- **按模型调参**：每个模型可单独设置上下文窗口与最大输出（支持 K/M tokens 紧凑表达），并可控制其在模型列表中的可见性；
- **生成期间可切换**：等待回复时即可排队切换模型，连续选择只生效最后一次；后台任务保持启动时的模型，不会中途换路。

### 2. 把真正相关的上下文交给 AI

KeepSeek 常驻 Secondary Sidebar。打开 `KeepSeek: Open Chat`、选择账号和模型后，就可以直接围绕当前项目提问：

- 在编辑器中选中代码，通过右键或 `Cmd+L` / `Ctrl+Shift+L` 加入对话；首次启动会自动补齐缺失的这两组快捷键（写入用户级 keybindings.json），无需手动配置；
...
- **批量命令执行**：一轮内提出的多条待确认命令可合并为一批，一次授权按序执行、逐条显示进度，并支持随时停止；授权随会话、审批模式或工作区信任变化而自动失效；
...
性能也从打开 VS Code 的第一秒开始：冷启动立即可用空会话，主模型、子代理模型与审批模式等设置启动即就绪；稳定的输入直接复用上下文用量估算缓存，显示更快、计算更省；并发的多个窗口写入互不影响；配合按会话固定的协议与工具 schema，多轮对话保持更高的提示缓存命中率，长任务的实际成本更低。
...
项目以 [MIT License](./LICENSE) 开源。如果 KeepSeek 帮到了你，欢迎 Star、点赞或把它分享给需要的朋友；任何 Issue 与改进建议都同样欢迎。
