...
- Goal 是 session 级 composer 模式，唯一入口位于 `/` 命令菜单末尾；`ChatSession.goalComposerMode` 只供宿主/UI 持久化，绝不进入 Provider projection。开启后输入框提交目标，宿主用已配置的 `proposal` 模型发起一次无工具严格 JSON 请求，同时完成适用性判定、必要规范化和 `GoalProposalV1` 生成；assessment/pending proposal 只存在 Extension Host 易失内存，失败回退宿主保守 proposal，只有用户在常驻确认卡或创建高级设置 Dialog 明确采纳后才创建 Goal/聊天消息。常驻区域位于 transcript 与 composer 之间：创建前显示确认卡，创建后互斥替换为进度/操作卡；运行态管理不再进入 Dialog，也不通过 `transcript.append` 伪装聊天消息。当前 attempt 仍以 view-only assistant 消息流式显示 Provider 推理/正文，既不写入 `ChatSession.messages`，也不参与 Provider replay。显式 Resume 必须先在卡片显示进行中状态，错误必须回传并重新启用操作；普通暂停/恢复只复核当前安全边界，不能重复执行 activation recovery 或再次失效当前证据。Goal、lease 与首个 v3 checkpoint 成功持久化后才发首个正式请求。一个 workspace 只允许一个现存 Goal（终态也必须先 Clear）；Stop 只进入终态而不清理，Clear 成功后才原子投影 `goal: null` 并退出绑定 session 的 Goal 模式，失败时保留终态卡和模式。
