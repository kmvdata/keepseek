...
const PRIMARY_SOURCE = { provider: 'deepseek', sourceId: 'primary' } as const;
const SECONDARY_SOURCE = { provider: 'deepseek', sourceId: 'secondary' } as const;
...
test('concurrent stores allow only one refresh claim and persist its reservation', async () => {
  ...
  try {
    ...
    const claims = await Promise.all([
      first.claimRefresh(NOW, INTERVAL_MS, true, PRIMARY_SOURCE),
      second.claimRefresh(NOW, INTERVAL_MS, true, PRIMARY_SOURCE)
    ]);
    ...
    assert.equal(
      await observer.isRefreshDue(NOW + 1, INTERVAL_MS, false, PRIMARY_SOURCE),
      false
    );

    const winner = claims[0] ? first : second;
    const other = claims[0] ? second : first;
    await winner.update({ totalBalance: 8.88, currency: '¥' }, NOW + 2, PRIMARY_SOURCE);

    // force still bypasses the completed throttle window once no request is in flight.
    assert.equal(await other.claimRefresh(NOW + 3, INTERVAL_MS, true, PRIMARY_SOURCE), true);
    await other.update({ totalBalance: 9.99, currency: '¥' }, NOW + 4, PRIMARY_SOURCE);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('a refresh claim fails closed when its reservation cannot be persisted', async () => {
  ...
  try {
    ...
    assert.equal(await store.claimRefresh(NOW, INTERVAL_MS, false, PRIMARY_SOURCE), false);

    mutableFs.writeFile = originalWriteFile;
    assert.equal(await store.claimRefresh(NOW + 1, INTERVAL_MS, false, PRIMARY_SOURCE), true);
    store.releaseRefreshClaim(PRIMARY_SOURCE);
  } finally {
    mutableFs.writeFile = originalWriteFile;
    rmSync(dir, { recursive: true, force: true });
  }
...
test('clear invalidates another store claim so its stale update cannot recreate the file', async () => {
  ...
  try {
    ...
    assert.equal(await requestStore.claimRefresh(NOW, INTERVAL_MS, true, PRIMARY_SOURCE), true);

    await settingsStore.clear(PRIMARY_SOURCE);
    await requestStore.update({ totalBalance: 8.88, currency: '¥' }, NOW + 1, PRIMARY_SOURCE);

    assert.equal(existsSync(join(dir, 'accounts', 'deepseek', 'primary', 'balance.json')), false);
    assert.equal(requestStore.getBalance(PRIMARY_SOURCE), undefined);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('delete invalidates another store claim so its stale update cannot resurrect source balance', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'keepseek-balance-'));
  try {
    const requestStore = new GlobalBalanceStore(vscode.Uri.file(dir));
    const settingsStore = new GlobalBalanceStore(vscode.Uri.file(dir));
    ...
    await settingsStore.deleteSource(PRIMARY_SOURCE);
    await requestStore.update({ totalBalance: 8.88, currency: '¥' }, NOW + 1, PRIMARY_SOURCE);
...
test('an abandoned caller can release its claim so force refresh remains available', async () => {
  ...
  try {
    ...
    await assert.rejects(async () => {
      assert.equal(await first.claimRefresh(NOW, INTERVAL_MS, true, PRIMARY_SOURCE), true);
      try {
        throw new Error('unexpected refresh failure');
      } finally {
        first.releaseRefreshClaim(PRIMARY_SOURCE);
      }
    }, /unexpected refresh failure/u);

    assert.equal(await second.claimRefresh(NOW + 1, INTERVAL_MS, true, PRIMARY_SOURCE), true);
    second.releaseRefreshClaim(PRIMARY_SOURCE);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('balance snapshots and throttle clocks are isolated by provider and source id', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'keepseek-balance-'));
  try {
    const store = new GlobalBalanceStore(vscode.Uri.file(dir));
    await store.update({ totalBalance: 8.88, currency: '¥' }, NOW, PRIMARY_SOURCE);
    await store.update({ totalBalance: 3.21, currency: '$' }, NOW - INTERVAL_MS, SECONDARY_SOURCE);
    await store.update(
      { totalBalance: 7.77, currency: '$' },
      NOW,
      { provider: 'openai-compatible', sourceId: 'primary' }
    );

    assert.equal(await store.isRefreshDue(NOW + 1, INTERVAL_MS, false, PRIMARY_SOURCE), false);
    assert.equal(await store.isRefreshDue(NOW + 1, INTERVAL_MS, false, SECONDARY_SOURCE), true);
    assert.equal(store.getBalance(PRIMARY_SOURCE)?.totalBalance, 8.88);
    assert.equal(store.getBalance(SECONDARY_SOURCE)?.totalBalance, 3.21);
    assert.equal(store.getBalance({ provider: 'openai-compatible', sourceId: 'primary' })?.totalBalance, 7.77);
...
test('selectSource scopes call sites without leaking another source snapshot', async () => {
  const { store, cleanup } = createStore();
  try {
    store.selectSource(PRIMARY_SOURCE);
    await store.update({ totalBalance: 10, currency: '¥' }, NOW);
    store.selectSource(SECONDARY_SOURCE);
    assert.equal(store.getBalance(), undefined);
...
test('source scope rejects path traversal ids before touching storage', async () => {
  const { store, cleanup } = createStore();
  try {
    await assert.rejects(
      store.update(
        ...
        NOW,
        { provider: 'deepseek', sourceId: '../outside' }
      ),
      /Invalid balance source id/u
    );
  } finally {
    cleanup();
  }
...
test('deleteSource physically removes only that source balance file', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'keepseek-balance-'));
  try {
    const store = new GlobalBalanceStore(vscode.Uri.file(dir));
    await store.update({ totalBalance: 5, currency: '¥' }, NOW, PRIMARY_SOURCE);
    await store.update({ totalBalance: 6, currency: '¥' }, NOW, SECONDARY_SOURCE);

    await store.deleteSource(PRIMARY_SOURCE);

    assert.equal(store.getBalance(PRIMARY_SOURCE), undefined);
    assert.equal(existsSync(join(dir, 'accounts', 'deepseek', 'primary', 'balance.json')), false);
    assert.equal(existsSync(join(dir, 'accounts', 'deepseek', 'secondary', 'balance.json')), true);
    assert.equal(store.getBalance(SECONDARY_SOURCE)?.totalBalance, 6);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('the default source can read and clear the legacy shared balance file', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'keepseek-balance-'));
  try {
